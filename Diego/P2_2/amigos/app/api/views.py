...
def list_amigos():
    ...
    con los campos 'id', 'name', 'lati', 'longi' y 'device'
    """
    amigos = Amigo.query.all()
    amigos_list = [{'id': amigo.id, 'name': amigo.name,
                    'lati': amigo.lati, 'longi': amigo.longi,
                    'device': amigo.device}
                   for amigo in amigos]
...
def list_devices():
    """
    Retorna un JSON con la lista de todos los dispositivos no nulos
    (ni cadenas vacías) en la base de datos.
    """
    # Obtenemos todos los amigos que tienen un device no nulo
    amigos = Amigo.query.filter(Amigo.device.isnot(None), Amigo.device != "").all()
    # Extraemos solo los dispositivos únicos
    devices = list(set([amigo.device for amigo in amigos]))
