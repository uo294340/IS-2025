...
def parse_headers(header_bytes):
    ...
    try:
        ...
        for linea in header_text.split('\n'):
            ...
            if linea.strip() == "": # Fin de las cabeceras
                break
    except Exception:
        pass
...
try:
    ...
    try:
        # CAMBIO 1: Extraer el número de mensajes
    ...
    # CAMBIO 2: Iniciar el bucle
    for i in range(1, num_mensajes + 1):
        print(f"\n--- Leyendo cabeceras del mensaje {i} ---")
        # CAMBIO 3: Usar TOP i 0 en lugar de RETR 1
        cmd = f"TOP {i} 0\r\n".encode('utf-8')
        print(f"C: TOP {i} 0")
        sc.sendall(cmd)
        
        # CAMBIO 4: Bucle de lectura de cabeceras
        cabeceras_bytes = b''
        while True:
            linea = sc.recv(4096)
            if linea.endswith(b'\r\n.\r\n'):
